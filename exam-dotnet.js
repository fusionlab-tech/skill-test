// .NET Core Programming Exam Questions - B2-C1 Level
const dotnetQuestions = [
    // Basic Questions (3) - B2 Level - More Technically Complex
    {
        question: "You're building a REST API for an e-commerce application using .NET Core. Users report that when they make concurrent requests to update product inventory, the stock levels sometimes show incorrect values. Here's the current implementation:\n\n<pre><code>[ApiController]\n[Route(\"api/[controller]\")]\npublic class ProductsController : ControllerBase\n{\n    private readonly ApplicationDbContext _context;\n    \n    [HttpPut(\"{id}/inventory\")]\n    public async Task&lt;IActionResult&gt; UpdateInventory(int id, [FromBody] InventoryUpdateRequest request)\n    {\n        var product = await _context.Products.FindAsync(id);\n        if (product == null)\n            return NotFound();\n        \n        product.Stock -= request.Quantity;\n        await _context.SaveChangesAsync();\n        \n        return Ok(product);\n    }\n}</code></pre>\n\nWhat's causing the inventory synchronization issues?",
    ...
    {
        question: "You're building a user authentication system for a banking application using .NET Core Identity. The system needs to handle JWT tokens, but users report that they get logged out unexpectedly and sometimes see other users' data. Here's the current implementation:\n\n<pre><code>[ApiController]\n[Route(\"api/[controller]\")]\npublic class AuthController : ControllerBase\n{\n    private readonly UserManager&lt;ApplicationUser&gt; _userManager;\n    \n    [HttpPost(\"login\")]\n    public async Task&lt;IActionResult&gt; Login([FromBody] LoginRequest request)\n    {\n        var user = await _userManager.FindByNameAsync(request.Username);\n        if (user != null &amp;&amp; await _userManager.CheckPasswordAsync(user, request.Password))\n        {\n            var token = GenerateJwtToken(user);\n            return Ok(new { token });\n        }\n        return Unauthorized();\n    }\n    \n    [HttpGet(\"profile\")]\n    public async Task&lt;IActionResult&gt; GetProfile()\n    {\n        var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;\n        var user = await _userManager.FindByIdAsync(userId);\n        return Ok(user);\n    }\n}</code></pre>\n\nWhat's causing the authentication and data isolation issues?",
    ...
    {
        question: "You're building a file upload service for a document management system using .NET Core. Users report that large files fail to upload and sometimes the upload process gets stuck. Here's the current implementation:\n\n<pre><code>[ApiController]\n[Route(\"api/[controller]\")]\npublic class FilesController : ControllerBase\n{\n    [HttpPost(\"upload\")]\n    public async Task&lt;IActionResult&gt; UploadFile(IFormFile file)\n    {\n        if (file == null || file.Length == 0)\n            return BadRequest(\"File is empty\");\n        \n        var fileName = file.FileName;\n        var filePath = Path.Combine(\"uploads\", fileName);\n        \n        using (var stream = new FileStream(filePath, FileMode.Create))\n        {\n            await file.CopyToAsync(stream);\n        }\n        \n        return Ok(\"File uploaded successfully\");\n    }\n}</code></pre>\n\nWhat's causing the file upload issues?",
    ...
    {
        question: "You're building a caching system for a high-traffic e-commerce application using .NET Core with Redis. The system needs to handle product catalog, user sessions, and real-time inventory updates. You're experiencing cache invalidation issues and stale data problems. Here's the current implementation:\n\n<pre><code>[ApiController]\n[Route(\"api/[controller]\")]\npublic class ProductsController : ControllerBase\n{\n    private readonly IMemoryCache _cache;\n    private readonly IDistributedCache _distributedCache;\n    private readonly ApplicationDbContext _context;\n    \n    [HttpGet(\"{id}\")]\n    public async Task&lt;IActionResult&gt; GetProduct(int id)\n    {\n        var cacheKey = $\"product_{id}\";\n        \n        if (!_cache.TryGetValue(cacheKey, out Product product))\n        {\n            product = await _context.Products.FindAsync(id);\n            if (product != null)\n            {\n                _cache.Set(cacheKey, product, TimeSpan.FromMinutes(30));\n            }\n        }\n        \n        return Ok(product);\n    }\n    \n    [HttpPut(\"{id}\")]\n    public async Task&lt;IActionResult&gt; UpdateProduct(int id, [FromBody] Product product)\n    {\n        _context.Products.Update(product);\n        await _context.SaveChangesAsync();\n        \n        // Invalidate cache\n        _cache.Remove($\"product_{id}\");\n        \n        return Ok(product);\n    }\n}</code></pre>\n\nWhat's causing the cache invalidation and data consistency issues?",
